...
describe('Test cases for messagePinned', () => {
  test('Test successful message pin in a channel message', () => {
 ...
 test('Test successful message react in a dm message', () => {
    ...
    expect(resMessagePin.status).toStrictEqual(badRequest);
  });
	...
	test('test for channel message already pinned', () => {
		const resRegister1 = postRegister('auth/register/v2', {
      email: 'jacky.wong1@unsw.edu.au',
      password: '123456789010',
      nameFirst: 'jacky',
    ...
    const resMessagePin1 = postMessagePinned('message/pin/v1', {
        token: resRegister1.bodyObj.token,
        messageId: resMessageSend.bodyObj.messageId
    })
    expect(resMessagePin1.status).toStrictEqual(OK);
    const resMessagePin2 = postMessagePinned('message/pin/v1', {
        token: resRegister1.bodyObj.token,
        messageId: resMessageSend.bodyObj.messageId
    })
    expect(resMessagePin2.status).toStrictEqual(badRequest);
  });
    ...
    expect(resMessagePin.status).toStrictEqual(restrictedAccess);
    });
