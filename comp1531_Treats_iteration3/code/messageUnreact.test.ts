...
// Helper function for message/unreact/v1
const postMessageUnreact = (path: string, body: {token: string, messageId: number, reactId: number}) => {
    const res = request(
      'POST',
      `${url}:${port}/${path}`,
      {
...
describe('Test cases for messageUnreact', () => {
  test('Test successful message unreact in a channel message', () => {
    ...
    const resMessageUnreact = postMessageUnreact('message/unreact/v1', {
        token: resRegister1.bodyObj.token,
        messageId: resMessageSend.bodyObj.messageId,
        reactId: 1
    })
		expect(resMessageUnreact.status).toStrictEqual(OK);
        expect(resMessageUnreact.bodyObj).toStrictEqual({});
 });
 test('Test successful message react in a dm message', () => {
    ...
    expect(resMessageUnreact.status).toStrictEqual(badRequest);
  });
	...
	test('test for channel message has no react to unreact', () => {
		const resRegister1 = postRegister('auth/register/v2', {
      email: 'jacky.wong1@unsw.edu.au',
      password: '123456789010',
      nameFirst: 'jacky',
	...
	test('test for dm message has no react to unreact', () => {
		const resRegister1 = postRegister('auth/register/v2', {
      email: 'jacky.wong1@unsw.edu.au',
      password: '123456789010',
      nameFirst: 'jacky',
    ...
    expect(resMessageUnreact.status).toStrictEqual(restrictedAccess);
	 });
